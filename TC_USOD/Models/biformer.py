...
# from timm.models.registry import register_model  # 这里timm版本不适配，所以更改了一下
...
class BiFormer(nn.Module):
    def __init__(self, depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        ##########################################################################
        #
        self.norm_0 = nn.BatchNorm2d(embed_dim[0])
        self.norm_1 = nn.BatchNorm2d(embed_dim[1])
        self.norm_2 = nn.BatchNorm2d(embed_dim[2])
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            x = self.stages[i](x)
            if i == 0:
                feature_map2 = x.deepcopy()
                feature_map2 = self.norm_0(feature_map2)
            if i == 1:
                feature_map3 = x.deepcopy()
                feature_map3 = self.norm_1(feature_map3)
            if i == 2:
                feature_map4 = x.deepcopy()
                feature_map4 = self.norm_2(feature_map4)
        x = self.norm(x)
        x = self.pre_logits(x)
        feature_map5 = x
    ...
    # 这里到底需不需要进行batch_norm呢,先加上试试

    def forward2(self, x):
        x = self.downsample_layers[0](x) # res = (56, 28, 14, 7), wins = (64, 16, 4, 1)
        x = self.stages[0](x)
        x = self.norm_0(x)
        return x
    def forward3(self, x):
        x = self.downsample_layers[1](x) # res = (56, 28, 14, 7), wins = (64, 16, 4, 1)
        x = self.stages[1](x)
        x = self.norm_1(x)
        return x
    def forward4(self, x):
        x = self.downsample_layers[2](x) # res = (56, 28, 14, 7), wins = (64, 16, 4, 1)
        x = self.stages[2](x)
        x = self.norm_2(x)
        return x
    def forward5(self, x):
        x = self.downsample_layers[3](x) # res = (56, 28, 14, 7), wins = (64, 16, 4, 1)
        x = self.stages[3](x)
        x = self.norm(x)
        return x
    ...
    def forward(self, x1, layer_flag=0):  # 新加的，还没来得及修改
        ...
        if self.FlagForward == 0:
            feature_map1, feature_map2, feature_map3, feature_map4, feature_map5 = self.forward_features(x1)
            return feature_map1, feature_map2, feature_map3, feature_map4, feature_map5
        elif self.FlagForward == 1:
            feature_map1 = self.forward1(x1)
            return feature_map1
        elif self.FlagForward == 2:
            feature_map2 = self.forward2(x1)
            return feature_map2
        elif self.FlagForward == 3:
            feature_map3 = self.forward3(x1)
            return feature_map3
        elif self.FlagForward == 4:
            feature_map4 = self.forward4(x1)
            return feature_map4
        elif self.FlagForward == 5:
            feature_map5 = self.forward5(x1)
            return feature_map5
...
def biformer_tiny(pretrained=False, pretrained_cfg=None,
    ...
    args = kwargs['args']
    if pretrained:
        state_dict = torch.load(args.biformer_pretrained_model)
        model.load_state_dict(state_dict['model'], strict=True)
        # load_checkpoint(model, args.biformer_pretrained_model, use_ema=True)
        print('Model loaded from {}'.format(args.biformer_pretrained_model))
