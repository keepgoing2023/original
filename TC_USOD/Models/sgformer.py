...
class SGFormer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            if i == 0:
                ...
                patch_embed = PatchMerging(dim=embed_dims[i - 1],  # 这里的PatchMerging主要起到一个降采样的作用。
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            x, H, W = patch_embed(x)
            if i == 0:
                x += self.pos_embed  # 这里如果是第一层的话就加上pos_embed`
            for blk in block:
                x, mask = blk(x, H, W, mask)
            x = norm(x)
            if i == 0:  # k
                feature_1_4 = x
                feature_map2 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            if i == 1:
                feature_1_8 = x
                feature_map3 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            if i == 2:
                feature_1_16 = x
                feature_map4 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            if i == 3:
                feature_1_32 = x
                feature_map5 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()  # g
            if i != self.num_stages - 1:
                x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            # stage 1: x(b, 96, 56, 56)
            # stage 2: x(b, 192, 28, 28)
            # stage 3: x(b, 384, 14, 14)
            # stage 4: x(b, 49, 768)
        return feature_map1, feature_map2, feature_map3, feature_map4, feature_map5, feature_1_4, feature_1_8, feature_1_16, feature_1_32
        # return x.mean(dim=1)

    def ori_forward(self, x):
        x = self.forward_features(x)  # x(b,768)
        x = self.head(x)  # x(b,1000)
    ...
    def forward2(self, x):
        ...
        block = getattr(self, f"block{1}")
        norm = getattr(self, f"norm{1}")
        x, H, W = patch_embed(x)
        x += self.pos_embed
        for blk in block:
            x, mask = blk(x, H, W, mask)
    ...
    def forward5(self, x):
        ...
        for blk in block:
            x, mask = blk(x, H, W, mask)
        x = norm(x)
        feature_map5 = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        return feature_map5, x

    def forward(self, x1, layer_flag=0):  # 新加的，还没来得及修改
        """
        @brief:
        """
        self.FlagForward = layer_flag
        if self.FlagForward == 0:
            feature_map1, feature_map2, feature_map3, feature_map4, feature_map5, feature_1_4, feature_1_8, feature_1_16, feature_1_32 = self.forward_features(x1)
            return feature_map1, feature_map2, feature_map3, feature_map4, feature_map5, feature_1_4, feature_1_8, feature_1_16, feature_1_32
        elif self.FlagForward == 1:
            feature_map1 = self.forward1(x1)
            return feature_map1
        elif self.FlagForward == 2:
            feature_map2, feature_1_4 = self.forward2(x1)
            return feature_map2, feature_1_4
        elif self.FlagForward == 3:
            feature_map3, feature_1_8 = self.forward3(x1)
            return feature_map3, feature_1_8
        elif self.FlagForward == 4:
            feature_map4, feature_1_16 = self.forward4(x1)
            return feature_map4, feature_1_16
        elif self.FlagForward == 5:
            feature_map5, feature_1_32 = self.forward5(x1)
            return feature_map5, feature_1_32
...
def sgformer_s(pretrained=False, **kwargs):
    ...
    # 照着t2t那个改的，后续如果有问题在进行处理
    args = kwargs['args']
    if pretrained:
        load_checkpoint(model, args.SG_former_pretrained_model, use_ema=True)
        print('Model loaded from {}'.format(args.SG_former_pretrained_model))
