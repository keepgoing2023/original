...
class TopkRouting(nn.Module):
    ...
    def forward(self, query:Tensor, key:Tensor)->Tuple[Tensor]:
        ...
        if not self.diff_routing:
            query, key = query.detach(), key.detach()  # detach() 的作用是创建一个新的张量，其值和原始张量相同，但不再与计算图关联，
                                                       # 不会计算梯度。这样做通常用于阻止梯度传播到这些张量，从而在反向传播过程中不更新它们的梯度。
        query_hat, key_hat = self.emb(query), self.emb(key) # per-window pooling -> (n, p^2, c) 
        attn_logit = (query_hat*self.scale) @ key_hat.transpose(-2, -1) # (n, p^2, p^2)
        topk_attn_logit, topk_index = torch.topk(attn_logit, k=self.topk, dim=-1) # (n, p^2, k), (n, p^2, k)
                                                      # 使用 PyTorch 的 torch.topk 函数对张量 attn_logit 进行按值的降序排列，并返回前 k 个最大值及其对应的索引。
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, n_win=7, qk_dim=None, qk_scale=None,
        ...
        self.qk_dim = qk_dim or dim  # 如果 qk_dim 的值为真（非零、非空），则将 self.qk_dim 的值设置为 qk_dim，否则将其设置为 dim。
        ...
        if self.param_attention == 'qkvo':
            ...
            self.qkv = QKVLinear(self.dim, self.qk_dim)
            self.wo = nn.Identity()  # self.wo 被设置为 nn.Identity()，这表示它是一个恒等映射，不进行任何操作。在 PyTorch 中，
                                     # nn.Identity() 是一个层（layer），但它的作用是将输入直接输出，没有任何变换。
        else:
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        lepe = self.lepe(rearrange(kv[..., self.qk_dim:], 'n (j i) h w c -> n c (j h) (i w)', j=self.n_win, i=self.n_win).contiguous())  # 通过 contiguous() 方法保证了新的张量是连续存储的。
