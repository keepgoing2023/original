...
class Averager():
    ...
    def add(self, v, n=1.0):
        self.v = (self.v * self.n + v * n) / (self.n + n)  # self.v * self.n 是当前平均值的总和，v * n 是要添加的新样本值的总和。将它们相加并除以总样本数 self.n + n 来计算新的平均值。
        self.n += n  # 将 n 的值增加，以反映新样本点的数量。
...
def log(obj, filename='log.txt'):
    ...
    if _log_path is not None:
        # 'a' 参数表示以追加（append）模式打开文件。这意味着如果文件已经存在，将在文件末尾追加新的内容；如果文件不存在，将创建一个新文件。
...
#     fmeasure, MAE, _, _ = calc_fmeasure(y_pred, y_true)  # kg
...
def calc_fmeasure(y_pred,y_true):
    ...
    with torch.no_grad():
        for i in range(batchsize):
            ...
            preds.extend(pred_float_.ravel())  # pred_float_.ravel() 是一个用于将 NumPy 数组扁平化（展平）的操作。具体来说，这个操作将多维数组变为一维数组，即将原始数组的所有元素按照行优先顺序排成一行
            gts.extend(gt_float_.ravel())

        RECALL = recall_score(gts, preds)
        PERC = precision_score(gts, preds)

        if PERC == 0 and RECALL == 0:  # k 为了防止出现0的情况
            fmeasure = 0.0
        else:
